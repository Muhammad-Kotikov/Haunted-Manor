...
# Berechnung der Skalierung, um den Bildschirm ohne schwarze Ränder zu füllen
if bg_aspect_ratio > screen_aspect_ratio:
    new_width = HEIGHT * bg_aspect_ratio
    background_image = pygame.transform.scale(background_image, (int(new_width), HEIGHT))
    bg_rect = background_image.get_rect(center=(WIDTH // 2, HEIGHT // 2))  # Zentrieren
else:
    new_height = WIDTH / bg_aspect_ratio
    background_image = pygame.transform.scale(background_image, (WIDTH, int(new_height)))
    bg_rect = background_image.get_rect(center=(WIDTH // 2, HEIGHT // 2))  # Zentrieren

# Sicherstellen, dass das Bild etwas höher am Bildschirm positioniert wird
bg_rect.y = -50  # Verschiebung nach oben, z.B. 50 Pixel
...
# Button-Liste (alle gleich groß und ganz links ausgerichtet)
buttons = [
...
# Zurück-Button mit transparentem PNG-Bild
...
back_button_image = pygame.transform.scale(back_button_image, (75, 75))  # Größe des Bildes anpassen
back_button_rect = back_button_image.get_rect(topleft=(20, 20))  # Position des Buttons

# Funktionen
def draw_menu():
    # Titel "Haunted Manor" mit Schatten
    title_shadow = TITLE_FONT.render("Haunted Manor", True, (0, 0, 0))
